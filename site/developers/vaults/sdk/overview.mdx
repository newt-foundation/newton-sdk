...
- **Vendor modules:** Typed wrappers for manager actions on Morpho, Morpho Blue, EulerEarn, EVault, and Superform. Vendor modules produce the calldata that the Shield forwards after policy approval.
...
VaultKit is for privileged vault operations such as reallocations, cap changes, and other curator or manager calls. End-user deposits and withdrawals continue through the vault protocol's normal UX; they do not route through Shield.
...
- Review [Vendor Modules](/developers/vaults/sdk/vendor-modules) to choose a typed integration or the generic-call escape hatch.
- Use the [Curator Guide](/developers/vaults/operations/curator-guide) for the full deploy, policy, role, delegate, bypass, ownership, and upgrade lifecycle.
