...
Morpho vault curators can route MetaMorpho manager actions through a Newton Shield before the vault receives the call. Your app still builds the Morpho action with the Morpho contracts or SDKs it already uses; VaultKit packages that exact call as an intent, asks Newton operators to evaluate the configured policy, and forwards the call only when the policy returns an approval attestation.
...
Newton environments are separate stacks. A `gw_` API key minted for `stagef` will return `401` against `prod`, and a `prod` key will not authenticate against `stagef`. Create the key from the Newton dashboard for the same environment your Shield uses.

## Install Packages

Every VaultKit Morpho integration needs the Shield SDK, Viem, and the policy packs you plan to enforce:
...
Install Morpho's SDK only if your app uses it to read vault state or build allocations:

```bash
pnpm add @morpho-org/blue-sdk-viem
```

If your app already has the MetaMorpho ABI and allocation logic, you can encode `reallocate` directly with `viem`.
...
With the pack-aware Shield client, encode the MetaMorpho calldata and call `sendCall(...)`:

```typescript
import { encodeFunctionData, maxUint256 } from 'viem'

const allocations = [
  { marketId: '0xMarketIdA' as `0x${string}`, assets: 1_000_000n },
  { marketId: '0xMarketIdB' as `0x${string}`, assets: maxUint256 },
]

const result = await shield.sendCall({
  to: '0xMetaMorphoVaultAddress',
...
For a Vaults.fyi + Chainalysis composite, include the per-call screening input your deployed pack expects under `prepareQueryOptions.chainalysis`. Omit it for a Vaults.fyi-only policy.
...
The SDK also exposes lower-level Morpho actions for direct `NewtonShield` extension flows. Use the pack-aware `sendCall(...)` path above unless you are intentionally building on the lower-level `NewtonShield` runtime.
...
| Type-check and encode | Your app compiles against the published package surface and produces MetaMorpho calldata. | Node, package install, and ABI or Morpho SDK. |
