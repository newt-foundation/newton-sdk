...
```typescript
import { NewtonTaskError, SDKError, SDKErrorCode } from '@magicnewton/newton-protocol-sdk';

try {
  await walletClient.evaluateIntentDirect(request);
} catch (error) {
  if (error instanceof NewtonTaskError) {
    // Task-level failure (e.g., quorum not reached)
    console.error('Task failed:', error.message);
    for (const opErr of error.operatorErrors) {
      console.error(`  Operator ${opErr.operator_address}: ${opErr.message} (retryable: ${opErr.retryable})`);
    }
