...
Sign encrypted identity data with EIP-712 and submit to the gateway via `newt_sendIdentityEncrypted`. The wallet client signs `EncryptedIdentityData { string data }` using EIP-712 domain `{name: "IdentityRegistry", version: "1", chainId, verifyingContract}`.

```ts
const result = await walletClient.sendIdentityEncrypted({
  identityOwner: '0xYourAddress',
  identityData: 'encrypted-blob',
  identityDomain: 'kyc',
});
```
...
|-----------|------|-------------|
| `identityOwner` | `Address` | The identity owner's address (must match the wallet signer) |
| `identityData` | `string` | The encrypted identity data blob |
| `identityDomain` | `IdentityDomainName \| string` | Domain name (e.g., `"kyc"`). Hashed to bytes32 via keccak256 |

The IdentityRegistry contract address is resolved automatically from the chain ID.

**Returns:**

| Field | Type | Description |
|-------|------|-------------|
| `inclusion_tx` | `string` | Transaction hash of the on-chain inclusion |

**Errors:**

- `MISSING_CHAIN` -- wallet client has no chain configured
- `MISSING_ACCOUNT` -- wallet client has no account configured
- `INVALID_ADDRESS` -- no IdentityRegistry address for the chain, or invalid `identityOwner`
- `IDENTITY_OWNER_MISMATCH` -- wallet signer does not match `identityOwner`
...
Sign encrypted identity data with EIP-712 and submit to the gateway. The gateway validates the signature and submits the data to the on-chain IdentityRegistry.
...
```ts
client.sendIdentityEncrypted(params: SendIdentityEncryptedParams): Promise<SendIdentityEncryptedResponse>
```
...
|-----------|------|-------------|
| `identityOwner` | `Address` | The identity owner's address (must match the wallet signer) |
| `identityData` | `string` | The encrypted identity data blob (opaque string) |
| `identityDomain` | `IdentityDomainName \| string` | Domain name (e.g., `"kyc"`). Hashed to bytes32 via keccak256 |
...
<Expandable title="SendIdentityEncryptedParams">
  ```ts
  interface SendIdentityEncryptedParams {
    identityOwner: Address;
    identityData: string;
    identityDomain: IdentityDomainName | string;
  }

  type IdentityDomainName = 'kyc';
  ```
</Expandable>

<Expandable title="SendIdentityEncryptedResponse">
  ```ts
  interface SendIdentityEncryptedResponse {
    inclusion_tx: string;
  }
  ```
</Expandable>
