...
These methods are available on the wallet client for submitting EIP-712 signed identity data to the on-chain IdentityRegistry. They are also exported as standalone functions from the package.
...
const result = await walletClient.sendIdentityEncrypted({
  ...
  identityDomain: 'kyc',
  identityRegistryAddress: '0xRegistryAddress',
});
```
...
| `identityRegistryAddress` | `Address` | IdentityRegistry contract address for the EIP-712 domain |
...
- `INVALID_ADDRESS` -- invalid `identityOwner` or `identityRegistryAddress`
