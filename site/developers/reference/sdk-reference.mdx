...
<Note>
  When a task fails (e.g., quorum not reached), `submitEvaluationRequest`, `evaluateIntentDirect`, and `submitIntentAndSubscribe` throw a `NewtonTaskError` that includes the `taskId` and `operatorErrors` array from the gateway response. Catch it with `instanceof NewtonTaskError` to inspect per-operator failure details. The `validate_calldata` field is available on the raw `GatewayCreateTaskResult` via the [RPC API](/developers/reference/rpc-api).
</Note>
...
<Expandable title="UploadIdentityEncryptedParams & Response">
  ```ts
...
<Expandable title="GetIdentityEncryptedResult">
  ```ts
...
<Expandable title="SecretsPublicKeyResponse">
  ```ts
...
<Expandable title="RegisterWebhookParams & RegisterWebhookResult">
  ```ts
...
<Expandable title="UnregisterWebhookResult">
  ```ts
...
<Expandable title="TaskFailureType">
  ```ts
...
<Expandable title="RegisterIdentityDataParams">
  ```ts
...
Thrown when a task submission fails at the application level (e.g., quorum not reached, operator evaluation failures). Carries per-operator error details from the gateway.

```ts
import { NewtonTaskError } from '@magicnewton/newton-protocol-sdk';

class NewtonTaskError extends Error {
...
Thrown by `submitEvaluationRequest`, `evaluateIntentDirect`, and `submitIntentAndSubscribe` when the gateway returns an application-level error.

```ts
try {
  const { result } = await walletClient.evaluateIntentDirect(request);
} catch (error) {
  if (error instanceof NewtonTaskError) {
    console.error('Task failed:', error.message);
    console.error('Task ID:', error.taskId);
    for (const opErr of error.operatorErrors) {
      console.error(`  Operator ${opErr.operator_address}: ${opErr.message}`);
    }
  }
}
```
...
<Note>
  `NewtonTaskError` and the `OperatorError` type are exported from the main package entrypoint. `SDKError`, `MagicRPCError`, and the error code enums are currently internal types — catch them generically using `instanceof Error`.
</Note>
