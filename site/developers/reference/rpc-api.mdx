...
### newt_sendIdentityEncrypted

{/* TODO (HPKE migration): This RPC method will be deprecated when identity data moves to
    off-chain HPKE storage. Post-migration, identity data is encrypted with HPKE and uploaded
    via newt_uploadIdentityEncrypted (returns { data_ref_id, gateway_signature, deadline }),
    with only a string data_ref_id stored on-chain via registerIdentityData.
    The Newton SDK does not wrap this RPC method — it is called directly by the newton-identity popup. */}

Submit EIP-712 signed encrypted identity data to the on-chain IdentityRegistry. The gateway validates the signature, then submits the data to the contract.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `identity_owner` | `Address` | Yes | The identity owner's address (must match the EIP-712 signer) |
| `identity_owner_sig` | `Hex` | Yes | EIP-712 signature over `EncryptedIdentityData { string data }` |
| `identity_data` | `object` | Yes | Object with a single `data` field containing the encrypted identity blob |
| `identity_data.data` | `string` | Yes | The encrypted identity data string |
| `identity_domain` | `Hex (bytes32)` | Yes | Identity domain hash — `keccak256` of the domain name (e.g., `keccak256("kyc")`) |
...
```json
{
  ...
  "version": "1",
  "chainId": "<chain_id>",
  "verifyingContract": "<IdentityRegistry_address>"
}
```

The EIP-712 struct type:

```
EncryptedIdentityData(string data)
```
...
- Authentication failure if the recovered signer does not match `identity_owner`
- Authorization failure if the API key lacks `RpcWrite` permission

See [Identity Policy Reference](/developers/verified-credential/identity-policy-reference) for writing policies that check identity data.
