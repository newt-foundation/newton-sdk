...
{/* TODO (HPKE migration): This RPC method will be deprecated when identity data moves to
    ...
    via newt_uploadEncryptedData, with only a 32-byte data_ref_id stored on-chain via
    registerIdentityDataRef. The Newton SDK does not wrap this RPC method — it is called
    directly by the newton-identity popup. */}
