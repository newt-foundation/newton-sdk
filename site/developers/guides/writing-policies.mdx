...
A Newton policy is a Rego program that evaluates whether an Intent should be approved. Policies reference data from two sources: configuration parameters (`data.params`) and runtime data from WASM oracles (`data.wasm`). For how the WASM oracle and Rego policy fit together, see the [Newton Mental Model](/developers/overview/mental-model).
