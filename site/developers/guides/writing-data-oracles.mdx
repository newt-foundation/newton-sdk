...
Data oracles are WebAssembly (WASM) components that fetch or compute external data at evaluation time. The Newton network executes your WASM, and the output is fed into your Rego policy as `data.wasm`. For how oracles fit alongside Rego policies, see the [Newton Mental Model](/developers/overview/mental-model).
