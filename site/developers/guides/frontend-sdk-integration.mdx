...
let evalResponse;
try {
  evalResponse = await walletClient.evaluateIntentDirect(evalRequest);
} catch (error) {
  if (error instanceof NewtonTaskError) {
    // Inspect per-operator failures (e.g., quorum not reached)
    console.error("Task failed:", error.message);
    for (const opErr of error.operatorErrors) {
      console.error(`  ${opErr.operator_address}: ${opErr.message}`);
    }
    throw error;
  }
  throw error;
