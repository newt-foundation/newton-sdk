...
This creates an association between the user's Newton identity wallet and your policy client contract in the `IdentityRegistry`. After this link is established, Newton's policy engine can access the user's registered KYC data when evaluating tasks submitted through your policy client.  A user's registered identity data can be reused across any application sharing the same identity domain, not just yours. Before calling `linkApp`, surface this to the user in plain language: linking grants your app access to the same KYC data other apps may already have, and other apps may gain access to data registered here.
...
Unlinking removes the user's identity association with your policy client. Once unlinked, any policy evaluation that depends on this link will fail until the user re-links. Make it easy for users to find and trigger this action in your app; identity links should be as easy to remove as they are to create.
