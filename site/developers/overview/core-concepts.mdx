...
A **Policy** is a Rego program that defines the conditions an Intent must meet to be approved. Policies are stored on IPFS as **raw Rego text** (referenced by CID) and evaluated at runtime by **Regorus**. Policies are reusable across multiple PolicyClients.
...
For how the WASM oracle and Rego policy fit together, see the [Newton Mental Model](/developers/overview/mental-model). You write policies in [Step 2 of the integration guide](/developers/guides/writing-policies). See the [Rego syntax guide](/developers/advanced/rego-syntax-guide) for language reference.
...
A **PolicyData** oracle is a WASM component that fetches or computes external data at evaluation time. You write it in JavaScript, Rust, or Python; what ships is the compiled WASM (JavaScript builds use `jco componentize`). The output is fed to the Rego policy as `data.wasm`. Examples include fetching token prices, checking sanctions lists, or verifying KYC status. See the [Newton Mental Model](/developers/overview/mental-model) and [Writing Data Oracles](/developers/guides/writing-data-oracles) to build one.
