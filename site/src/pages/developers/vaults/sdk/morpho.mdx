...
# Morpho Guide [How to gate MetaMorpho curator actions with Newton VaultKit and composite policy packs.]
...
Every VaultKit Morpho integration needs VaultKit, policy core, Viem, the policy packs you plan to enforce, and Morpho's SDK:
...
Use `definePolicy().with(...)` with the modules deployed in your Newton policy. A Vaults.fyi-only policy looks like this:
...
Create one client for the curator, vault, chain, policy, and version slot:
...
`createShield(...)` is idempotent. If a previous run did not finish writing policy params, use `attachWithoutVerify: true` only for a recovery run that immediately calls `setParams(...)`, then remove the flag and call `reverify()`.
...
| Setup stages | `definePolicy`, `createShield`, `setParams`, and `uploadSecrets` work. | Funded curator key and Newton/API secrets. |
...
| Module-set mismatch or composite policy mismatch | The modules declared with `definePolicy().with(...)` do not match the policy address. | Use a policy deployed with exactly the selected modules. |
...
<Card title="Getting Started" icon="code" to="/developers/vaults/sdk/integration-guide">
  Install VaultKit, configure a Shield, and execute a policy-gated vault action.
</Card>

<Card title="Errors" icon="triangle-alert" to="/developers/vaults/sdk/errors">
