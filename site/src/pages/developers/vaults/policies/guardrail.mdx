...
Install the pack package, add it with `definePolicy(...).with(guardrail)`, and upload `GUARDRAIL_API_KEY` only when your deployment requires it. Per-call inputs go under `prepareQueryOptions.guardrail`.
