...
Install the pack package, add it with `definePolicy(...).with(persona)`, upload the API key with `client.uploadSecrets(...)`, and pass any per-call identity input its `prepareQuery` requires under `prepareQueryOptions.persona`.
