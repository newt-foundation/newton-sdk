...
Install the pack package, add it with `definePolicy(...).with(balancer)`, and pass any per-call inputs its `prepareQuery` requires under `prepareQueryOptions.balancer`.
