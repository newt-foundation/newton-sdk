...
Simulates full Rego policy evaluation (`newt_simulatePolicy`). Tests a policy client's whole policy set against a sample intent. May require ownership if a policy in the set uses stored secrets.
...
const result = await walletClient.simulatePolicy({
  ...
  // one entry per policy in the set, positionally aligned; '0x' for a pure-Rego policy
  wasmArgs: ['0x'],
});
...
Simulates a policy's WASM oracle execution (`newt_simulatePolicyData`) with caller-provided secrets. No ownership verification is required.
...
Simulates a policy's WASM oracle execution (`newt_simulatePolicyDataWithClient`) using stored secrets for a policy client. Requires ownership of the policy client.
...
| `wasmCid`        | `string`     | Yes               | CID of the WASM oracle; empty string for a pure-Rego policy              |
| `secretsSchemaCid` | `string`   | Yes               | CID of the oracle's secrets schema; empty unless `wasmCid` is set        |
...
Sets the secrets schema CID on the policy contract. Owner only.

```ts
const txHash = await walletClient.setSecretsSchemaCid({
  secretsSchemaCid: 'bafy...',
});
```
...
Encrypts plaintext secrets client-side with HPKE and uploads the envelope to the gateway for a policy client's policy. The gateway's X25519 public key is fetched automatically if not provided.
...
Returns the IPFS CID of the policy's WASM oracle plugin. Empty for a pure-Rego policy.

```ts
const wasmCid = await publicClient.getWasmCid();
```
...
Returns the IPFS CID of the JSON schema that the oracle's secrets must conform to.

```ts
const schemaCid = await publicClient.getSecretsSchemaCid();
```
...
const policyId = publicClient.precomputePolicyId({
  ...
  revision: 1,
  policies: [
    { policy: '0x...', config: { policyParams: '0x...', expireAfter: 300 } },
  ],
});
```
...
:::details[PolicyId, PolicyInfo, PolicyCodeInfo]
  ```ts
...
:::details[PolicyParamsJson, SetPolicyInput, SetPolicyResult]
  ```ts
