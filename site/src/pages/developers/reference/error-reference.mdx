...
# Error Reference [Newton Protocol error codes, messages, and resolution steps]
...
|------|------|-------------|------------|
| -32615 | PolicyEvaluationError | Rego policy evaluation failed | Check Rego syntax, verify `data.wasm` and `data.params` are populated correctly |
...
| `"Missing secrets for policy {addr}"` | Secrets not uploaded for the policy | Upload secrets via `newt_storeEncryptedSecrets` |
...
- **`newton-cli policy simulate-wasm`** — test WASM oracle execution locally
- **`newt_simulatePolicy`** — test policy evaluation via the Gateway
- **`newt_simulatePolicyData`** — test WASM execution via the Gateway
- **[Newton Explorer](https://explorer.newton.xyz)** — view task status and evaluation results
