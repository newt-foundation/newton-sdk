...
Create `~/.newton/newton-cli.toml` for non-secret persistent defaults:
...
Most commands accept configuration through environment variables or command-line arguments. Inject credentials into the current process environment or the user-level `~/.newton/.env`; do not commit or source a project-local `.env`:
...
export PINATA_GATEWAY="your_pinata_gateway"
```

Never paste secret values into chat with a coding agent or place them directly in command arguments, where they can leak through shell history and process inspection.
...
Runs a policy's WASM oracle locally, without deploying anything.

**Parameters**

| Flag | Required | Description |
| --- | --- | --- |
| `--wasm-file` | Yes | Path to the compiled oracle WASM file |
| `--input-json` | Yes | JSON input for the simulation |

```bash
newton-cli --chain-id 84532 policy simulate-wasm \
...
Deploys a policy contract with the specified policy CIDs.
...
```bash
newton-cli policy deploy --policy-cids policy-files/policy_cids.json
```
...
:::note
`policy simulate` delegates to operators for the full data pipeline. Use it to debug Rego logic before going live.
:::
...
Upload HPKE-encrypted secrets for WASM data providers that call `secrets::get()` at runtime. Secrets are scoped to a policy's address. When you redeploy a policy it gets a new address.
...
| `--policy-client` | Yes | PolicyClient contract address |
| `--policy-address` | Yes | Policy contract address |
| `--api-key` | Yes | Newton API key |
...
```bash
# Create a secrets file matching your policy's secrets schema
...
The `regorus` command wraps Newton's fork of **Regorus 0.9.0**, configured for **Rego v1**, with Newton-specific crypto extensions (`newton.crypto.ecdsa_recover_signer`, `newton.crypto.ecdsa_recover_signer_personal`) and identity built-ins (`newton.identity.*`, `newton.confidential.*`).

Its compatibility target is an **OPA v1.2.0-compatible subset plus Newton-specific extensions**. It does not implement every OPA built-in. See the [Rego Syntax Guide](/developers/advanced/rego-syntax-guide#unsupported-opa-built-ins) before relying on standard crypto/JWT, GraphQL, JSON Patch, network/CIDR/DNS, AWS provider, metadata, or module-parsing functions.
...
:::note
The `--non-strict` flag enables OPA-style non-strict evaluation. It does not enable missing OPA built-ins or imply complete OPA v1.2.0 compatibility.
:::
...
A typical local-first workflow for integrating a new smart contract with Newton:

| Step | Command | Purpose |
| --- | --- | --- |
| 1 | `newton-cli policy scaffold` | Create the policy source, schemas, fixtures, and deployment config |
| 2 | `newton-cli policy build -p <dir>` | Validate and compile the policy artifacts |
| 3 | `newton-cli policy simulate -p <dir>` | Prove at least one allow and one deny case locally |
| 4 | `newton-cli policy deploy -p <dir>` | Generate CIDs and deploy the policy after approval |
| 5 | `forge build && forge test` | Build and adversarially test the application PolicyClient |
| 6 | `forge script ... --broadcast` | Deploy the tested PolicyClient after approval |
| 7 | `cast send ... setPolicyAddress/setPolicy` | Bind policy and params while the local deployer is owner |
| 8 | `cast send ... registerClient` | Register only when the application requires it |
| 9 | `cast send ... setPolicyClientOwner` | Transfer ownership to the dashboard login wallet when required |
| 10 | `newton-cli secrets upload` | Upload client-scoped secrets after owner and schema checks |
| 11 | SDK `evaluateIntentDirect` | Sign the EIP-712 intent and evaluate through a server-side gateway client |
...
:::details[IncompatiblePolicyVersion]
  Your policy was deployed with an older factory. Deploy a new policy via the latest factory, then follow the [deployment guide](/developers/guides/deploying-with-cli#step-6--bind-policy-and-parameters) to bind it while an available signer still owns the PolicyClient. See [Version Compatibility](/developers/guides/smart-contract-integration#version-compatibility).
:::
## Next Steps
