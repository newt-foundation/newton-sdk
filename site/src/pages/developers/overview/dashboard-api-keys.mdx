...
### Use the API key
    ...
    The same key also reads task and policy data from the [Explorer API](/developers/reference/explorer-api):
    ```bash
    curl https://explorer.api.newton.xyz/v1/task/<task_id> \
      -H "Authorization: Bearer <your_api_key>"
    ```
...
<Card icon="activity" to="/developers/reference/explorer-api" title="Explorer API">
  Query tasks and policies with your API key
</Card>
