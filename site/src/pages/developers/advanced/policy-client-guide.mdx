...
const intent = {
    ...
    functionSignature: stringToHexBytes("withdraw(uint256,address)")
};

// 2. Read the PolicyClient EIP-712 domain and sign the exact intent
const domain = await readEip712Domain(vaultAddress, publicClient);
if (!domain) {
    throw new Error("This policy client does not expose an EIP-712 domain.");
}
...
For `functionSignature`, use `"0x"` when `data` is `"0x"` (for example, a native ETH transfer). For contract calls, hex-encode the human-readable ABI signature without the `function ` prefix, such as `withdraw(uint256,address)`. The intent signature domain should come from the deployed PolicyClient's EIP-5267 `eip712Domain()` method when available; if you hardcode it, `name` and `version` must match the contract's `EIP712(name, version)` constructor values and `verifyingContract` must be the PolicyClient address.
...
:::details[Simple Token Transfer Protection]
    ...
    ```
  :::
  :::details[Vault with Multiple Operations]
...
:::details[Decode Intent Data]
    ...
    ```
  :::
  :::details[Verify Function Selector]
  ...
  :::details[Execute Raw Intent]
    ...
    ```
  :::
  :::details[Multi-Attestation Batching]
    ...
    ```
  :::
  :::details[Conditional Validation]
...
### Update your client and reconfigure
    ```solidity
    // Point to new policy
...
:::details[Error: "IncompatiblePolicyVersion"]
  ...
  :::
  :::details[Error: "PolicyClientNotRegistered"]
  ...
  :::
  :::details[Error: "Policy ID does not match"]
  ...
  :::
  :::details[Error: "Not authorized intent sender"]
  ...
  :::
  :::details[Error: "Chain ID does not match"]
  ...
  :::
  :::details[Error: "Attestation already spent"]
  ...
  :::
  :::details[Error: "InterfaceNotSupported"]
    ...
    **Solution**: Override `supportsInterface` correctly:
    ...
    ```
  :::
  :::details[Gas Estimation Failures]
