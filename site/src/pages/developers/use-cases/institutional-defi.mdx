...
# Institutional DeFi [Enable institutional crypto adoption with programmable compliance policies. Newton Protocol provides verifiable transaction authorization for DeFi protocols, fund managers, and regulated financial products on Ethereum.]
...
allow if {
    data.wasm.current_protocol_exposure + input.intent.value <= data.params.max_protocol_exposure
}
```
...
allow if {
    not data.wasm.is_sanctioned
    data.wasm.jurisdiction_allowed
    input.intent.value <= data.params.single_transaction_limit
    data.wasm.daily_volume + input.intent.value <= data.params.daily_limit
}
```
...
<Card icon="file-code" to="/developers/guides/writing-policies" title="Write Compliance Policies">
