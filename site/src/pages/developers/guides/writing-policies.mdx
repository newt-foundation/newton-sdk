...
description: "Write Rego policies for blockchain transaction authorization with Newton Protocol. Examples for spend limits, allowlists, sanctions screening, and fraud prevention rules."
keywords:
  - rego policy examples
  - blockchain transaction policy
...
:::info
Newton supports **Rego v1 syntax** through its fork of **Regorus 0.9.0**. The runtime is an **OPA v1.2.0-compatible subset plus Newton-specific extensions**, not a complete implementation of every OPA built-in. Review the [Rego Syntax Guide](/developers/advanced/rego-syntax-guide#language-and-engine-compatibility) before porting an existing OPA policy.
:::
...
| `data.wasm` | WASM oracle | Runtime data returned by this policy's WASM oracle (prices, KYC status). Packs namespace their output under a pack id — see [Reading a pack's output](#reading-a-packs-output) |
...
:::note
Your own oracle's fields appear directly under `data.wasm` (e.g. `data.wasm.price`). A [policy pack](/developers/guides/policy-packs) namespaces its output under a pack id (`data.wasm.<pack-id>.price`) — see [Reading a pack's output](#reading-a-packs-output).
:::

## Reading a pack's output

A policy carries at most one WASM oracle. When that oracle is a [policy pack](/developers/guides/policy-packs), it wraps its output under a **pack id**, so your Rego reads `data.wasm.<pack-id>.*` and the thresholds you configure under `data.params.<pack-id>.*`:
...
To gate on a second signal, give it its own policy in the client's set — see [Composing Multiple Policies](/developers/guides/chaining-data-oracles) for how a set is evaluated and the deny-set pattern.
