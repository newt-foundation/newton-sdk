...
- **Oracle sidebar** — pick the data oracle this policy runs. A policy carries at most one; gate on more signals by [composing several policies](/developers/guides/chaining-data-oracles). Packs namespace their output under a pack id, so reference it in Rego as `data.wasm.<pack-id>.*`.
...
2. Deploys the `NewtonPolicy` contract carrying the selected oracle's `wasmCid` and `secretsSchemaCid`.
3. Binds the policy to your PolicyClient and records the deployed addresses.

This is the recommended way to deploy a [policy set](/developers/guides/chaining-data-oracles) today, since it deploys each policy and writes the ordered set onto your client in one flow.
