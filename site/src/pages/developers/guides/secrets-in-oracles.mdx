...
description: "Declare, upload, and read API keys inside Newton policy oracles. JSON Schema declaration, HPKE-encrypted upload via SDK or CLI, and the newton:provider/secrets WASM host interface."
keywords:
  - newton secrets
  - policy data secrets
  - wasm oracle api key
...
# Uploading & Accessing Secrets in Oracles [Declare, upload, and read API keys inside Newton policy oracles. JSON Schema declaration, HPKE-encrypted upload via SDK or CLI, and the newton:provider/secrets WASM host interface.]

Policy oracles often need an API key to fetch external data. Newton lets you store those secrets **encrypted** so they are never exposed on-chain, and decrypts them only inside the operator at evaluation time — your WASM oracle reads them through a host interface without ever handling the ciphertext.
...
Each policy's oracle declares the secrets it requires as a JSON Schema, stored on-chain as the oracle's `secretsSchemaCid`. Uploaded secrets are validated against this schema before being stored.
...
:::warning
Secrets are scoped to a policy's address. When you redeploy a policy it gets a new address.
:::
...
async function storeSecrets() {
  const client = createWalletClient({
    ...
    apiKey: 'test-api-key',
    policyContractAddress: '0x1234567890123456789012345678901234567890',
  }))

  const result = await client.storeEncryptedSecrets({
    policyClient: '0x1234567890123456789012345678901234567890' as `0x${string}`,
...
- `get()` returns a `result<secret-response, string>`; `secret-response.value` is `list<u8>` — the bytes of the decrypted secrets JSON object. The scope (which PolicyClient + policy) is bound by the operator's execution context, so you do not pass any identifiers.
...
- Fail closed: if a fetch errors, return a namespaced `error` so your Rego can deny on it (see [Composing Multiple Policies](/developers/guides/chaining-data-oracles#writing-rego-for-one-policy-in-the-set)).
...
```bash
# Verify stored secrets work for a deployed policy
...
<Cards>
<Card icon="lock" to="/developers/advanced/encrypting-secrets" title="Encrypting Secrets">
