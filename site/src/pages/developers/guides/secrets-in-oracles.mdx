...
# Uploading & Accessing Secrets in Oracles [Declare, upload, and read API keys inside Newton PolicyData oracles. JSON Schema declaration, HPKE-encrypted upload via SDK or CLI, and the newton:provider/secrets WASM host interface.]
...
async function storeSecrets() {
  ...
  const result = await client.storeEncryptedSecrets({
    ...
    policyDataAddress: '0x2222222222222222222222222222222222222222' as `0x${string}`,
