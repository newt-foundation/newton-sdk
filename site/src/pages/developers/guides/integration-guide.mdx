...
# Integration Guide [End-to-end guide for adding transaction policies to your smart contracts with Newton Protocol. Deploy Rego policies, build WASM data oracles, and verify attestations on-chain.]
...
<Tabs>
  ...
  <Tab title="Build with an agent">
    Start with the [Agent Quickstart](/developers/overview/agent-quickstart). Newton Agent Skills turn a brief into locally tested policy and contract artifacts, preserve state in handoff files, and pause before deployment, gateway evaluation, and live transactions.
  </Tab>
</Tabs>

:::note
The agent and manual routes produce the same Newton components. The agent route adds resumable `policy-handoff.json`, `client-handoff.json`, and `demo-config.json` files; it does not bypass local simulation, Foundry tests, or deployment approvals.
:::
...
| **newton-cli**                         | 3     | Uploading policy files, generating CIDs, deploying policies                            |
| **Pinata account**                     | 3     | IPFS pinning (you will need a JWT and a gateway URL)                                   |

:::details[Install all tooling (Steps 1–5)]
  ```bash
  # Rust (includes cargo)
  ...
  # newton-cli (via newtup version manager)
...
<Card icon="database" to="/developers/guides/writing-data-oracles" title="Writing Data Oracles">
...
<Card icon="terminal" to="/developers/overview/agent-quickstart" title="Build with an Agent">
  Use a product brief to coordinate the same policy, PolicyClient, and frontend flow
</Card>
...
:::details['command not found' after installing tools]
    ...
    ```
:::
:::details[Policy build fails during componentization]
  Run the CLI toolchain check and install any missing componentization packages it reports:
  ...
  Re-run `newton-cli policy build -p <policy-dir>` after fixing the reported dependency or source error.
:::
:::details[InvalidAttestation error (0xbd8ba84d)]
...
:::
:::details[ExecutionFailed error (0xacfdb444)]
  ...
  - Malformed calldata
:::
:::details[WebSocket connection fails]
  ...
  ```
:::
:::details[Policy evaluation times out]
...
:::
:::details[Find out why a live task was non-compliant]
  Fetch the task from the [Explorer API](/developers/reference/explorer-api) with your Newton API key, using the `task_id` returned by the Gateway:
  ...
  - `error` or `operator_errors` set: the data oracle or evaluation failed. Test the oracle with `newton-cli policy-data simulate`.
  - `policy_task_data` shows the oracle output the policy evaluated. Replay it with `newt_simulateTask` to see which rule rejected the intent.
:::
## Factory Pattern

:::details[Advanced: Factory pattern for deploying multiple PolicyClients]
