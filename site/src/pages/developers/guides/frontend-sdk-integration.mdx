...
# Frontend SDK Integration [Build a Next.js frontend that submits Newton Protocol evaluation requests and executes attested transactions]
...
import { createPublicClient, createWalletClient, http, webSocket, Hex } from "viem";
import { privateKeyToAccount } from "viem/accounts";
import { sepolia } from "viem/chains";
import { newtonWalletClientActions } from "@newton-xyz/sdk";
const SEPOLIA_ALCHEMY_URL = "https://eth-sepolia.g.alchemy.com/v2/demo";
const SEPOLIA_ALCHEMY_WS_URL = "wss://eth-sepolia.g.alchemy.com/v2/demo";
const demoPrivateKey = '0x' + '1'.repeat(64) as Hex;
...
export const useNewtonClient = (privateKey: Hex, apiKey: string) => {
  const client = useMemo(() => {
    ...
    const walletClient = baseWalletClient.extend(
      newtonWalletClientActions({ apiKey })
    );

    return { walletClient, publicClient: basePublicClient, account };
  }, [privateKey]);
...
export const createEvaluationRequest = ({
  ...
  return {
    ...
    intent: {
      ...
      data: data,
      chainId: sepolia.id,
      // Use "0x" for a native ETH transfer or any intent with no calldata.
      // For a contract call, pass stringToHexBytes("transfer(address,uint256)").
...
The `functionSignature` value is for the intended transaction in `intent.data`, not for the later Newton wallet execution helper. For example:

```typescript
const nativeEthIntent = createEvaluationRequest({
  signerAddress,
...
const erc20TransferIntent = createEvaluationRequest({
  signerAddress,
  ...
  functionSignature: stringToHexBytes("transfer(address,uint256)"),
  wasmArgs,
});
```
...
```typescript
const domain = await readEip712Domain(evalRequest.policyClient, publicClient);
if (!domain) {
  throw new Error(
...
const intentSignature = await walletClient.signTypedData({
  ...
  message: {
    ...evalRequest.intent,
    value: BigInt(evalRequest.intent.value),
    chainId: BigInt(evalRequest.intent.chainId),
  },
});

// Evaluate intent — returns attestation data
const evalResponse = await walletClient.evaluateIntentDirect({
  ...evalRequest,
  intentSignature,
});
...
Add this EIP-5267 helper near your wallet/public client setup:
...
export async function readEip712Domain(policyClient: Address, publicClient) {
  try {
...
:::warning
The `functionSignature` field in the intent is the **hex-encoded human-readable ABI**, not the 4-byte selector. Use `"0x"` when `data` is `"0x"`. For contract calls, the gateway needs the full signature to decode calldata into `decoded_function_arguments` for Rego. Using the 4-byte selector (`0xa9059cbb`) instead causes decoding to fail silently.
...
// Correct for native ETH transfer or no calldata
const noCalldataFunctionSig = "0x";
