...
# Technical Whitepaper [Technical specification of Newton Protocol — an EigenLayer Actively Validated Service (AVS) for policy-based authorization of blockchain transactions. Covers cryptographic primitives, consensus, and security model.]
...
For protocol vision, market context, and use cases, see the [Newton Protocol Whitepaper](https://newton.xyz/whitepaper). For developer integration details (JSON-RPC API, simulation modes, attestation validation paths), see the [RPC API Reference](/developers/reference/rpc-api) and the [Smart Contract Integration Guide](/developers/guides/smart-contract-integration).
