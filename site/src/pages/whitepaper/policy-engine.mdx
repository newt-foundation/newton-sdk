...
Newton's policy engine supports **Rego v1 syntax** through Newton's fork of **Regorus 0.9.0**. Its compatibility target is an **OPA v1.2.0-compatible subset plus Newton-specific extensions**. Newton extends the engine with privacy-preserving operations and cryptographic primitives for cross-chain signature verification and hash computation, but it does not implement every OPA built-in. Existing OPA policies and libraries must be checked for unsupported standard crypto/JWT, GraphQL, JSON Patch, network/CIDR/DNS, AWS provider, metadata, and module-parsing functions before use.
